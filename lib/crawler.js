...
Crawler.prototype.start = async function() {
  ...
  if (self.interval == undefined) self.interval = 20
  ...
  if (self.parseImages == undefined) self.parseImages = false
  if (self.parseForms == undefined) self.parseForms = false
  ...
  do {
    try {
      const qItem = self.queue.fetch()
      console.log(qItem)
      await sleep(self.interval)
      await self.crawl(qItem)
    } catch (e) {
      console.log(e)
    }
...
Crawler.prototype.crawl = async function(queueItem) {
  ...
  return await Promise.map(fetchedResources.links, async function(cur) {
    ...
    if (!self.externalLinks && !self.initialURL.includes(uri.hostname()))
      return Promise.reject(new Error()).reflect(`External Link - ${queueItem.href}`)
